...
export default function UserCart(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await props.axiosInstance.post("/api/cart/fetchCart");
        if (response.data.success) {
          setStatus(1);
          setCartProduct(response.data.cart);
          setCouponCode(
            response.data.cart[0].couponCode !== "No Code Applied"
              ? response.data.cart[0].couponCode
              : ""
          );
        }
      } catch (error) {
  ...
  const deleteProd = async (cart_item_id) => {
    setLoadingScreen(1);
    try {
      const response = await props.axiosInstance.post(
  ...
  const handleCouponRemove = async (e) => {
    setLoadingRemoveCoupon(1);
    e.preventDefault();
    try {
      const response = await props.axiosInstance.post("/api/cart/removeCoupon");
      ...
      <div className="container">
        <div className="row">
          <div className="mt-3 col-12 col-md-4 order-md-last">
            ...
            <button
              className="btn w-100 mt-2 btn-dark"
